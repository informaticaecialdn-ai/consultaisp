 ...
 */
import { REGRAS_PADRAO, TIPOS_DE_REGRA, type RegrasAntiFraude } from "@shared/antifraude-regras";
...
export interface AvaliacaoFuga {
  ...
  criterios?: { tipo: string; atendido: boolean }[];
}
...
export function parseDataContrato(valor?: string): Date | null {
  ...
  if (br) {
    ...
    const valido = d.getUTCFullYear() === ano && d.getUTCMonth() === mes - 1 && d.getUTCDate() === dia;
    return valido ? d : null;
  }

  const partes = texto.match(/^(\d{4})-(\d{2})-(\d{2})(?:$|T| )/);
  if (!partes) return null;
  const [ano, mes, dia] = partes.slice(1).map(Number);
  const civil = new Date(Date.UTC(ano, mes - 1, dia));
  if (civil.getUTCFullYear() !== ano || civil.getUTCMonth() !== mes - 1 || civil.getUTCDate() !== dia) return null;
...
export function avaliarRiscoDeFuga(cliente: ClienteParaAvaliar, opcoes: OpcoesDeAvaliacao): AvaliacaoFuga {
  ...
  const idade = inicio ? diasDesde(inicio, opcoes.agora) : undefined;
  const diasDeContrato = idade !== undefined && idade >= 0 ? idade : undefined;
  const motivos: MotivoFuga[] = [];

  const inadimplente = regras.ativo_inadimplente;
  if (
    inadimplente.ativo &&
    Number.isFinite(cliente.totalOverdueAmount) && cliente.totalOverdueAmount > 0 &&
    Number.isFinite(cliente.maxDaysOverdue) &&
  ...
  const porTipo = { ativo_inadimplente: "divida_ativa", contrato_novo: "contrato_novo", consultas_repetidas: "consultas_repetidas", ativo_qualquer: "cliente_ativo" } as const;
  const criterios = TIPOS_DE_REGRA.filter(t => regras[t].ativo).map(tipo => ({ tipo, atendido: motivos.includes(porTipo[tipo]) }));
  if (motivos.length === 0) return { alerta: false, motivos: [], descartadoPor: "nenhuma_regra", diasDeContrato, criterios };
  if (regras.combinacao === "todas" && criterios.some(c => !c.atendido)) {
    return { alerta: false, motivos, descartadoPor: "criterios_incompletos", diasDeContrato, criterios };
  }
  return { alerta: true, motivos, diasDeContrato, criterios };
}
