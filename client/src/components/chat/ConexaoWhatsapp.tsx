...
export function ConexaoWhatsapp({ provider, podeAdministrar }: { provider: ProvedorWhatsapp; podeAdministrar: boolean }) {
  ...
  // O estado real ao abrir: "conectado" ou "aguardando" antes de qualquer clique —
  // e o QR que a instancia ja tiver aparece sem o operador pedir outro.
  useEffect(() => { if (podeAdministrar && !oficial) acao.mutate("consultar"); }, []); // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(() => {
  ...
  return <div className="space-y-3 border-t border-[var(--border)] pt-4" data-testid="chat-conexao-whatsapp">
    <div className="flex flex-wrap items-center gap-2"><h4 className="text-sm font-medium">Conexão · {provider === "EVOLUTION" ? "WhatsApp da plataforma" : provider}</h4>{estado && <SeloCobranca tom={estado.connected && estado.loggedIn ? "ok" : "gated"}>{estado.connected && estado.loggedIn ? "conectado" : estado.status === "connecting" ? "aguardando pareamento" : "desconectado"}</SeloCobranca>}</div>
    ...
    {estado?.aviso && <p role="status" className="rounded border border-[var(--gated-border)] bg-[var(--gated-bg)] p-2 text-xs leading-5 text-[var(--gated)]" data-testid="chat-conexao-aviso">{estado.aviso}</p>}
