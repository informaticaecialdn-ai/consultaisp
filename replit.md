...
- **Anti-Fraud Module:** Calculates a risk score (0-100) using weighted factors such as days overdue, overdue amount, unreturned equipment value, recent ISP consultations, and contract age. It includes `Alerts` with detailed cards, `Risk Score` rankings, `Patterns` for fraud detection, a mock `AI Analysis` for risk summaries, customizable `Rules` with SE/ENTAO conditions, and `Configurations` for various thresholds and multi-channel notifications (WhatsApp, Email, Push, SMS) persisted via `/api/provider/notification-settings`.
- **Equipment Management (`/importacao-equipamentos`):** Allows ISPs to register unreturned equipment manually (form) or via CSV import (drag-and-drop with papaparse preview). Equipment records include CPF/CNPJ, type, brand, model, serial number, value, and status (retido/recuperado/em_cobranca/baixado). Equipment is cross-referenced in ISP consultations to alert receiving providers. Data stored in `equipamentos` table.
- **ISP Consultation UX Improvements:** Loading screen shows 4 named stages (Validando documento → Consultando ERPs → Analisando historico → Calculando score). Cost estimate banner appears when CPF/CNPJ is detected, showing credit model. New "Consulta em Lote" tab allows batch querying of multiple documents via textarea, with results table showing score, risk, credits consumed, and status per document.
...
- **Heat Map (Mapa de Calor):** The heat map has two data sources: (1) **IXC ERP Cache** (`server/heatmap-cache.ts`) — fetches ALL delinquent customers from each provider's IXC ERP (`fn_areceber` endpoint, status=A), geocodes by CEP/city/state via ViaCEP + Nominatim, stores in-memory per-provider cache with 24h TTL, refreshed automatically every 24h via scheduler (`startHeatmapCacheScheduler()`), manually via `POST /api/heatmap/refresh`; (2) **ISP Consultation Network** — extracted from JSONB of past ISP consultations for cross-provider data. The `customers` table is NOT used for heat map data (only used for dashboard stats and equipment import context). IXC data is never stored locally — only in-memory cache.
...
- **Cadastros (Registration Management):** A dedicated "Cadastros" tab in the superadmin panel (`/admin-sistema#cadastros`) shows all provider registrations with their verification status (Pendente/Aprovado/Rejeitado). Features include: status filter cards with counts, search by name/email/CNPJ, detailed registration info (admin user, email verification, CNPJ, subdomain, plan, date), and actions to approve/reject/revert status. Uses `verificationStatus` field on the `providers` table.
- **Superadmin Functionality:** A dedicated `/admin-sistema` panel provides comprehensive system management, including provider and user administration, registration verification, financial oversight, and support chat management.
