...
// Bia realoca leads parados. Usada por src/workers/supervisor.js.
...
module.exports = {
  ...
  description:
    'Reatribui leads parados (sem resposta ha N dias) de volta pra Sofia (nurturing). Util pra Bia liberar slots do time comercial. Retorna contagem de reatribuidos por agente origem.',
  input_schema: {
    type: 'object',
    properties: {
      dias_parado: {
    ...
    const tx = db.transaction(() => {
      for (const lead of stuck) {
        db.prepare(
          ...
          `REASSIGN (Bia): parado ${dias}d em ${lead.agente_atual} -> sofia nurturing`,
          lead.id
        );
        db.prepare(
          `INSERT INTO handoffs (lead_id, de_agente, para_agente, motivo)
           VALUES (?, ?, 'sofia', ?)`
        ).run(lead.id, lead.agente_atual, `Bia realocou: parado ${dias}d`);
        stats.reassigned++;
