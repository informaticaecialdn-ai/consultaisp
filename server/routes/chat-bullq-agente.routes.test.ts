...
describe("o webhook de volta", () => {
  ...
  it("assinatura certa: atualiza a conversa e grava na linha do tempo que a IA transferiu", async () => {
    ...
    expect(armazem.registrarEventoDeCobranca).toHaveBeenCalledWith(42, expect.objectContaining({ casoId: 10, tipo: "nota", notas: "Chat: a IA transferiu a conversa ao atendente" }));
  });
  it("atendente assumiu: status OPEN e a nota certa; sem conversa, ignora", async () => {
    ...
    expect(armazem.registrarEventoDeCobranca.mock.calls.at(-1)![1].notas).toBe("Chat: um atendente assumiu a conversa");
